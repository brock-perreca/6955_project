...
> **Two-machine parallel ablation.** This same-day diagnosis was run
> *independently* on Brock-Asus-Laptop and Brock-O11. Both confirmed
> the kinematic-ceiling hypothesis. The Asus track used
> `walker2d_hipopen.xml` (`[-30, 60]`, permissive both directions);
> the O11 track used `walker2d_hiprelax.xml` (`[-150, 35]`, +5°
> headroom only) and is documented as **Batch 4b — Tier 0 hiprelax
> ablation** below. The two variants together bracket the residual
> reward bottleneck. See also
> [`docs/TIER0_DIAGNOSTICS.md`](TIER0_DIAGNOSTICS.md) for the O11
> Tier 0 ledger that drove the hiprelax variant.
...
## Batch 4b — 2026-04-29 — Tier 0 morphology ablation (hiprelax variant, Brock-O11)

> **Companion to Batch 4 above.** Same hypothesis, run on the second
> machine, with a *minimal-headroom* hip range instead of
> hipopen's wide bracket. Both confirmed the kinematic ceiling; the
> two variants together bracket how much of the residual gap is
> reward-driven.

### Setup

Before launching the planned reward-structure reform, the O11
machine paused for a Tier 0 diagnostic asking the same question
Batch 4 (hipopen) answered: was the stiff-hip basin
**reward-driven** (as Batch 3's overnight concluded) or
**morphology-driven**?

Two static probes settled the framing before any retraining (these
are the same observations Batch 4's "Diagnosis" section made, with
...
- **A.2 (xvel-5M deterministic rollout, 5 seeds × 600 steps).** hip_r
  ...
  pushing into the soft-constraint wall.

See [`docs/TIER0_DIAGNOSTICS.md`](TIER0_DIAGNOSTICS.md) for the full
A.1/A.2/C ledger.

**The Batch 4b experiment (C in the Tier 0 ledger):** single-knob
...
no looser — the hypothesis was that an overly-permissive limit
would create a new bad basin like overswing/kicking, which is exactly
what hipopen does). Otherwise verbatim xvel-5M recipe.
...
Tooling shipped on the O11 machine in this batch (analogous to and
compatible with the Asus-laptop Batch 4 tooling — both machines
arrived at the same `--xml` flag and `xml_file` env_kwargs.json
persistence independently):
- `--xml` CLI flag in `ppo_walker2d_phase.py` (Asus laptop also
  added the same flag in commit 7724ff9; merged into one
  implementation).
- `env_kwargs.json` written by training records `xml_file`;
...
- `scripts/tier0/evaluate_C.py` — full validation pipeline for the
  three-seed comparison (dashboards, eval_biomech, mp4s, comparison
  panel, summary).
...
Compared to Batch 4's hipopen variant: hipopen *overshoots* (hip ROM
91.5° at 2M / 63° at 5M against a 43° target); hiprelax *undershoots*
(17–20°). The hipopen track has more permissive geometry so the
policy fully escapes the wall and runs aggressively; the hiprelax
track sits just below the reference peak, so the policy tracks
reference shape but doesn't fully extend.
...
Tier 1 reward reform — `forward_reward = exp(-3·(v-1.25)²)` + drop
`xvel_term` floor — on top of a **relaxed-hip MJCF**. The cleanest
experimental design runs it on **both** `walker2d_hipopen.xml` and
`walker2d_hiprelax.xml`:
...
- if both stay where they are, the trap is deeper than reward+range
  and we look at gait-cycle frame rate / phase observation as Tier 2.
...
```
# render_phase.py auto-loads the trained-against MJCF from each run's
# env_kwargs.json. No --xml needed.
python src/walker2d/render_phase.py --live results/restart_b4_hiprelax_s11:final results/restart_b4_hiprelax_s12:final results/restart_b4_hiprelax_s13:final

# Pre-rendered (Tier 0 C panel + per-seed mp4s):
docs/figures/tier0/C_hiprelax/C_hip_trace_comparison.png
docs/figures/tier0/C_hiprelax/00_reference_replay.mp4
docs/figures/tier0/C_hiprelax/xvel-5M_final.mp4
docs/figures/tier0/C_hiprelax/hiprelax_s11_final.mp4
docs/figures/tier0/C_hiprelax/hiprelax_s12_final.mp4
docs/figures/tier0/C_hiprelax/hiprelax_s13_final.mp4

# Full pipeline (dashboards x ckpts, eval_biomech, mp4s, comparison panel, summary):
