...
|---|---|---|
| [`biomech_report.py`](biomech_report.py) | Render a writeup-ready markdown table + 6-panel matplotlib figure from one or more `eval_biomech` JSONs. Overlays sim curves on the Ulrich reference (hip / knee / ankle / vGRF / hip-knee phase plane / stride-period bars). | Every writeup pass: `python scripts/biomech_report.py results/<run>_eval.json --rerollout` |
