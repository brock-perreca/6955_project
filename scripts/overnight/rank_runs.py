...
Composite score (higher is better):
    ...
    score = clip(ep_len_steps__median / 2500, 0, 1)            # survival
          ...
          + clip(1 - hip_knee_dtw / 0.30, 0, 1)                # shape fidelity (2-joint)
          + clip(1 - lr_stride_asymmetry / 0.30, 0, 1)         # symmetry
          + clip(1 - swing_drag_frac, 0, 1)                    # no toe-drag
          + clip(1 - all_joints_dtw / 1.0, 0, 1)               # full-body shape (Phase 4)

Each term is in [0, 1]; max score = 6.0. Reference cycle is the Subject 1
baseline (1.25 m/s, ~1.12 s stride period, ~17 strides per 20s window).

The `all_joints_dtw` term (Phase 4 extension) catches stand-and-wiggle
exploits that the 2-joint hip-knee DTW misses: a stiff-hip + knee-wiggle
gait scores low on hip-knee but high on all-six because the full
reference trajectory isn't tracked.
...
def _score(summary: dict) -> tuple[float, dict]:
    ...
    aj_dtw = g("all_joints_dtw__median", math.nan)
    full_shape = 0.0 if math.isnan(aj_dtw) else _clip(1.0 - aj_dtw / 1.0, 0, 1)
    ...
    breakdown = {
        ...
        "full_shape": round(full_shape, 3),
    }
    return round(survival + cadence + shape + sym + no_drag + full_shape, 3), breakdown
...
def write_markdown(rows: list[dict], path: Path) -> None:
    ...
    lines.append(
        "Composite score is a sum of 6 terms in [0,1]: survival, "
        "cadence-match, hip-knee DTW (2-joint shape), L-R symmetry, "
        "no-toe-drag, all-joint DTW (full-body shape). "
        "Max = 6.0. Higher is better. **This is a starting point for "
        "your visual review, not a verdict.** Watch the top 5 MP4s "
        "before trusting the ranking."
    )
    lines.append("")
    lines.append("| rank | name | score | survival | cadence | shape | sym | no_drag | full_shape | preview |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|")
    for i, r in enumerate(rows, 1):
        ...
        lines.append(
            ...
            f"{t['no_drag']:.2f} | {t.get('full_shape', 0):.2f} | [mp4]({r['preview']}) |"
        )
    lines.append("")
